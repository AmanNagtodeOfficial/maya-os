#ifndef MAYA_GRAPHICS_H
#define MAYA_GRAPHICS_H

#include <stdint.h>

// Enhanced Graphics Functions for Ubuntu GUI
void maya_graphics_init(void);
void maya_set_pixel(int x, int y, uint8_t color);
uint8_t maya_get_pixel(int x, int y);
void maya_fill_rect(int x, int y, int width, int height, uint8_t color);
void maya_draw_rect(int x, int y, int width, int height, uint8_t color);
void maya_draw_line(int x1, int y1, int x2, int y2, uint8_t color);
void maya_fill_circle(int cx, int cy, int radius, uint8_t color);
void maya_draw_circle(int cx, int cy, int radius, uint8_t color);

// Text Rendering
void maya_draw_char(int x, int y, char c, uint8_t color, uint8_t bg_color);
void maya_draw_string(int x, int y, const char* str, uint8_t color, uint8_t bg_color);
void maya_draw_string_centered(int x, int y, int width, const char* str, uint8_t color, uint8_t bg_color);

// Icon Drawing
void maya_draw_folder_icon(int x, int y, int size);
void maya_draw_app_icon(int x, int y, int size, const char* app_name);
void maya_draw_file_icon(int x, int y, int size);
void maya_draw_system_icon(int x, int y, const char* icon_name, uint8_t color);

// Effects
void maya_draw_shadow(int x, int y, int width, int height);
void maya_draw_glow(int x, int y, int width, int height, uint8_t color);
void maya_apply_transparency(int x, int y, int width, int height, float alpha);

// Color Utilities
uint8_t maya_rgb_to_color(uint8_t r, uint8_t g, uint8_t b);
void maya_color_to_rgb(uint8_t color, uint8_t* r, uint8_t* g, uint8_t* b);
uint8_t maya_darken_color(uint8_t color, float factor);
uint8_t maya_lighten_color(uint8_t color, float factor);
...
void maya_animation_start(maya_animation_t* anim, int start, int end, uint32_t duration);
void maya_animation_update(maya_animation_t* anim);
uint8_t maya_animation_finished(maya_animation_t* anim);
