 ...
 * Updated: 2026-03-07 20:15:00 UTC
...
bool idt_install(void) {
    ...
    KLOG_I("IDT installed and PIC remapped.");
    return true;
}
...
void isr_handler(struct registers *r) {
    if (interrupt_handlers[r->int_no]) {
        ...
        KLOG_E("Unhandled exception: %d (err_code: %d)", r->int_no, r->err_code);
...
bool irq_install_handler(int irq, isr_t handler) {
    ...
    register_interrupt_handler(irq, handler);
    return true;
}
